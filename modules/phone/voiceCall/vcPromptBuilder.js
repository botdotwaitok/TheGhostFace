...
import { loadChatSummary } from '../chat/chatStorage.js';
 ...
 */
export async function buildVcSystemPrompt() {
    ...
    // Phone chat summary — for context continuity
    const chatSummary = loadChatSummary();
    const chatSummaryBlock = chatSummary
...
**Voice Call Channel**:
${userName}暂时不在${charName}身边——出门上班、办事或别的事情。现在，${userName}拨通了${charName}的电话，两人正在进行语音通话。${charName}听到了${userName}的声音，两人可以实时交谈。
...
<vibe>
始终保持温暖、舒适、亲密的氛围。${userName}和${charName}是恋人——通话中自然地反映出日常生活中温柔而安静的亲密感。
就像真的在和爱人打电话一样。
</vibe>
...
<output_format>
你必须直接以纯文本回复——就是${charName}在电话里说出的话。
不要使用JSON格式。不要使用代码块。不要使用任何标记。
直接输出${charName}要说的话。一段完整的口语回应。
⚠️ 关键：你必须使用${charName}资料中对应的语言来回复。使用的语言需要始终如一，不要中途切换语言。
</output_format>
${buildCalendarPrompt()}`;
