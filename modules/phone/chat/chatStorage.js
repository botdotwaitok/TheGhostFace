...
export function injectChatSummaryToST(summary) {
    console.warn(`${CHAT_LOG_PREFIX} injectChatSummaryToST is deprecated, use sendSummaryAsUserMessage`);
}

/** @deprecated Sending is now handled by sendSummaryAsUserMessage (via send button click) */
export async function triggerSTGenerate() {
    console.warn(`${CHAT_LOG_PREFIX} triggerSTGenerate is deprecated, use sendSummaryAsUserMessage`);
}

/**
 * Clear the phone chat sync extension prompt from ST context.
 * Called after the ST LLM has processed the summary.
 */
export function clearChatSyncPrompt() {
    try {
        const context = getContext();
        if (typeof context.setExtensionPrompt === 'function') {
            context.setExtensionPrompt('gf_phone_chat_sync', '', 1, 0);
            console.log(`${CHAT_LOG_PREFIX} Cleared phone chat sync prompt`);
        }
    } catch (e) {
        console.warn(`${CHAT_LOG_PREFIX} Failed to clear sync prompt:`, e);
    }
}
