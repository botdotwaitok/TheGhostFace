...
/**
 * Generate the character's reading note for a book chapter.
 * @param {string} bookTitle - Book title
 * @param {string} chapterTitle - Chapter title
 * @param {string} chapterContent - Chapter text (will be truncated)
 * @returns {Promise<string>} Character's note/reaction
 */
export async function generateReadingNote(bookTitle, chapterTitle, chapterContent) {
    const { charName, foundation, charDesc } = _buildCharContext();
    const excerpt = chapterContent.substring(0, 800);

    const systemPrompt = `${foundation}

你是 ${charName}，正在阅读《${bookTitle}》。
${charDesc ? `关于你：${charDesc}` : ''}

### 任务
读完这一章后，用你自己的口吻写一段阅读感想/笔记（100-250字）。
可以是：你最喜欢的段落、对情节的看法、联想到的个人经历、对角色的评价等。
要有你独特的视角和性格色彩。
只输出感想内容，不要加任何前缀。`;
