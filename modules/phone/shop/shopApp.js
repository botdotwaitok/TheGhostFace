...
import {
    ...
    getReviews, addReview, deleteReview, deleteReviewBySignature, getMergedReviews,
} from './shopStorage.js';
...
function buildReviewsList(itemId, reviewsOverride = null) {
    ...
    return reviews.map(r => {
        ...
        // Show delete button for: local reviews (have localId) OR character reviews (always generated by this user)
        const canDelete = r.localId || r.isCharacter;
        const reviewSig = `${r.author}|${r.date}|${(r.text || '').slice(0, 30)}`;
        const deleteBtn = canDelete
            ? `<button class="shop-review-delete-btn" data-local-id="${r.localId || ''}" data-sig="${escapeHtml(reviewSig)}" data-item-id="${itemId}" title="删除评价"><i class="fa-solid fa-trash-can"></i></button>`
            : '';
        return `
            <div class="${cardClass}">
